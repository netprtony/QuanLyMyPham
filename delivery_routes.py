...
def location_list():
    ...
    return render_template('delivery.html', locations=locations, enumerate=enumerate)
...
# Sửa---------------------------------------------------------
@delivery_bp.route('/update_delivery', methods=['POST'])
def update_location():
    ...
    return redirect(url_for('location_bp.delivery_list'))


# Xóa-------------------------------------------------------
@delivery_bp.route('/delete_delivery/<location_id>', methods=['POST'])
def delete_location(location_id):
    # Xóa khách hàng từ MongoDB
    result = location_collection.delete_one({'location_id': location_id})

    if result.deleted_count > 0:
        return redirect(url_for('location_bp.delivery_list'))  # Chuyển hướng về danh sách khách hàng
    else:
        return "Không tìm thấy khách hàng với ID này", 404


@delivery_bp.route('/filter-locations', methods=['GET'])
def filter_locations():
    ...
    return render_template('delivery_list.html', locations=filtered_locations)